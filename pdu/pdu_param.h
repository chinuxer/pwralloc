#pragma once
#include "pdu_broker.h"
#define KEYSTRLEN 20
typedef struct
{
    uint32_t value;
    char key[KEYSTRLEN];
} KeyValue_Pair;

typedef struct
{
    size_t count;
    uint16_t crc;
    KeyValue_Pair pairs[];
} KeyValue_Array;

typedef union
{
    uint32_t all;
    struct
    {
        uint32_t contactor : 24;
        uint32_t plug : 8;
    } id_of;
} Combo_Koinon;

#define ADD_KEYVALUE(...)                                             \
    ({                                                                \
        struct kv_pair                                                \
        {                                                             \
            char *key;                                                \
            uint32_t val;                                             \
        } pairs[] = {__VA_ARGS__};                                    \
        KeyValue_Array *array = NULL;                                 \
        for (size_t i = 0; i < sizeof(pairs) / sizeof(pairs[0]); i++) \
            array = add_KeyValue(pairs[i].key, pairs[i].val);         \
        array;                                                        \
    })

#define ADD_INTEGER(...)                                   \
    ({                                                     \
        int values[] = {__VA_ARGS__};                      \
        char names[] = #__VA_ARGS__;                       \
        KeyValue_Array *array = NULL;                      \
        size_t count = sizeof(values) / sizeof(values[0]); \
        char *saveptr = NULL;                              \
        char *token = strtok_r(names, ", ", &saveptr);     \
        for (size_t i = 0; i < count && token; i++)        \
        {                                                  \
            array = add_KeyValue(token, values[i]);        \
            token = strtok_r(NULL, ", ", &saveptr);        \
        }                                                  \
        array;                                             \
    })
#ifdef __INSTANT__PARAM__

static KeyValue_Pair make_keyvalue(const char *key, uint32_t val)
{
    KeyValue_Pair pair;
    pair.value = val;
    strncpy(pair.key, key, 20);
    return pair;
}
static KeyValue_Array *oprt_TopoParam(KeyValue_Array *param)
{
    static KeyValue_Array *gparam IN_PDU_RAM_SECTION = NULL;
    if ((bool)param)
    {
        gparam = param;
    }
    return gparam;
}
// cstat !MISRAC2012-Rule-8.7
KeyValue_Array *add_KeyValue(const char *key, uint32_t val)
{
    static KeyValue_Array *array IN_PDU_RAM_SECTION = NULL;
    if(NULL==key&&0==val)
    {
        array = NULL;
        return array;
    }
    size_t new_count = (array == NULL) ? 1 : array->count + 1;
    size_t new_size = sizeof(KeyValue_Array) + new_count * sizeof(KeyValue_Pair);

    // printf("key %s:%d\r\n", key, val);
    // cstat !MEM-free-no-alloc
    // cstat !MISRAC2012-Dir-4.13_g
    // cstat !CERT-EXP36-C_b
    KeyValue_Array *new_array = pdu_realloc(array, new_size);
    if (new_array == NULL)
    {
        return NULL;
    }

    new_array->pairs[new_count - 1] = make_keyvalue(key, val);
    new_array->count = new_count;
    array = new_array;

    return oprt_TopoParam(new_array);
}

static uint32_t find_KeyValue(const KeyValue_Array *array, const char *key)
{
    // cstat !MISRAC2012-Rule-14.3_b
    if (!(bool)array || !(bool)key)
    {
        return 0;
    }
    for (size_t i = 0; i < array->count; i++)
    {
        if (strncmp(array->pairs[i].key, key, KEYSTRLEN) == 0)
        {
            return array->pairs[i].value;
        }
    }
    return 0;
}

static void scan_KeyValue(Combo_Koinon *container, const KeyValue_Array *array, ID_TYPE plug_id)
{
    static size_t offset IN_PDU_RAM_SECTION = 0;
    if (!(bool)container || !(bool)array || !(bool)plug_id)
    {
        offset = 0;
        return;
    }
    char key[KEYSTRLEN];
    int len = snprintf(key, sizeof(key), "u32gun%02" PRIu32, plug_id);
    if (8 != len)
    {
        return;
    }
    for (size_t i = 0; i < array->count; i++)
    {
        if (strncmp(array->pairs[i].key, key, KEYSTRLEN) == 0)
        {
            (container + offset)->id_of.plug = plug_id;
            (container + offset)->id_of.contactor = array->pairs[i].value;
            offset++;
        }
    }

    return;
}

#define IS_CHAR(c) _Generic((c), char *: 1, signed char *: 1, unsigned char *: 1, default: 0)

#define PICKOUT_KEYVALUE(param, token) ({uint32_t val=find_KeyValue(param,token); val; })

static bool serialize_Download(KeyValue_Array *param_array)
{
    static const uint8_t aucCRCHi[] = {
        };
    static const uint8_t aucCRCLo[] = {
        };

#define macrocrc16(data, crc)                                            \
    do                                                                   \
    {                                                                    \
        crc = ((uint16_t)(aucCRCHi[(crc ^ data) & 0xff]) ^ (crc >> 8)) | \
              ((uint16_t)(aucCRCLo[(crc ^ data) & 0xff]) << 8);          \
    } while (0)

    if (NULL == param_array)
    {
        return false;
    }
    uint16_t crc = 0xFFFF;
    for (size_t i = 0; i < param_array->count * sizeof(KeyValue_Pair); i++)
    {
        // cstat !MISRAC2012-Rule-10.1_R6
        macrocrc16(*((uint8_t *)(param_array->pairs) + i), crc);
    }
    param_array->crc = crc;
    printf("Calculated CRC16: 0x%04X\n", crc);

    return true;
}

static bool deserialize_Upload(const void *data)
{
    return true;
    /*
#define PARSE_KEYVALUE_ARRAY(token) uint32_t token = PICKOUT_KEYVALUE(param_ref, #token)
    KeyValue_Array *param_ref = (KeyValue_Array *)data;
    if (!param_ref)
    {
        return false;
    }
    PARSE_KEYVALUE_ARRAY(se01);

    return true;
*/
}
#else
static bool serialize_Download(KeyValue_Array *param_array);
static bool deserialize_Upload(const void *data);
void *pdu_calloc(size_t size);
KeyValue_Array *add_KeyValue(const char *key, uint32_t val);
#endif